-- Simple Auth Setup - Clean Version
...
CREATE POLICY "Users own agents" ON agents FOR ALL USING (user_id = auth.uid()::TEXT);
...
CREATE POLICY "Users own datasets" ON datasets FOR ALL USING (user_id = auth.uid()::TEXT);
...
CREATE POLICY "Users own jobs" ON jobs FOR ALL USING (user_id = auth.uid()::TEXT);
